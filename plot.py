...
def add_shape(shp_path, ax, proj, edgecolor="grey6", facecolor="grey6", alpha=0.7, lw=0.8, lc="grey6", zorder=1.0):
    from cartopy.io.shapereader import Reader
    reader = Reader(shp_path)
    shape = cfeature.ShapelyFeature(reader.geometries(), proj, edgecolor=edgecolor, facecolor=facecolor, alpha=alpha)
    ax.add_feature(shape, linewidth=lw, zorder=zorder)
...
def generate_marker_list(style, color):
    ncolor = len(color)
    nstyle = len(style)
    marker_style = [style[i//ncolor] for i in range(ncolor*nstyle)]
    marker_color = [color[i%ncolor] for i in range(ncolor*nstyle)]
    marker_list = [marker_style, marker_color]
    return(marker_list)
      

def taylor_diagram(ax,r,std,color="r", labels=False, **kargs):
    dotlables=["" for _ in range(len(r))]
    args = {"dotlables":dotlables, "ifmarker":False, "mksize":8, "ifnumbers":False}
    args = {**args, **kargs}
    if np.min(r)<0:
        ...
        for i in np.arange(0,len(r)):
            if args["ifmarker"]:
                ax.plot(np.arccos(r[i]), std[i], marker=args["markerlist"][0][i],color=args["markerlist"][1][i],markersize=args['mksize'], label='{}'.format(args["dotlables"][i]), markeredgecolor=args["markeredgecolor"],markeredgewidth=args["markeredgewidth"])

            if args["ifnumbers"]:
              if labels:
                  ax.plot(np.arccos(r[i]), std[i], 'o',color=color,markersize=0, label='{} {}'.format(i+1,args["dotlables"][i]))
              else:
                  ax.plot(np.arccos(r[i]), std[i], 'o',color=color,markersize=0)
              ax.text(np.arccos(r[i]), std[i], s='{}'.format(i+1), c=color,fontsize=10, ha='center', va='center')
...
"""
调整了taylor_diagram的坐标字体大小等的一些参数，待改
circle_list为好模式列表（需要加粗）
dotlabel为所有点的label
"""
def taylor_diagram2(ax,r,std,circle_list,dotlabels,**kargs):
    # dotlables=["" for _ in range(len(r))]
...
def contour_label(ax, contourf_instance, **kargs):
  ...
  for num_line,line in enumerate(contourf_instance.collections):
    for path in line.get_paths():
      ...
      # print(min_ind)
      label_pos.append(vert[min_ind+num_line//2-len(contourf_instance.collections)//2,:])
  ax.clabel(contourf_instance, manual=label_pos, **kargs)
  

def contour_label_modified(ax, contourf_instance, threshold=12, **kargs):
  label_pos = []
  # point_num = []
  # x0y0 = []
  for num_line,line in enumerate(contourf_instance.collections):
    for path in line.get_paths():
      # 获取contour线的每一点的坐标
      vert = path.vertices
      # point_num.append(np.shape(vert)[0])
      # 获取每一条线的x坐标和y坐标
      if np.shape(vert)[0] >= threshold:
        x = vert[:,0]
        y = vert[:,1]
        # x0y0.append(vert[0,:])
        grad = (y[1:]-y[:-1])/(x[1:]-x[:-1]+1e-14)
        # 确定梯度最接近0的点的索引值
        min_grad_ind = np.argmin(abs(grad))+1
        label_pos.append(vert[min_grad_ind,:])
  ax.clabel(contourf_instance, manual=label_pos, **kargs)
  # for i in range(len(contourf_instance.collections)):
    # ax.text(x0y0[i][0], x0y0[i][1], s=str(point_num[i]), transform=ax.transData)